...
A time series of the seagrass percent cover was conducted between 2016 and 2018 (see @fig-Maps). A consistent cover of seagrass can be observed at Fort Espagnol since the beginning of the time series. From 2019 onwards, the meadows appear to become denser at that site, especially in the northern part of the mudflat, with the exception of 2020, when green algae covered the meadow in August. Concerning Kerouarc'h, the meadow was absent from the mudflat between 2016 and 2019. As for Fort Espagnol, in 2020 ICE CREAMS predicted almost exclusively green algae in Kerouarc'h. From 2020 onward, the seagrass meadows have seen a rapid extension, covering almost the entirety of the mudflat since 2021, and then becoming denser in 2022. 
...
knitr::include_graphics("../Output/Figs/Figure1.png")
  ...
  for(site_i in 1:length(unique(mask$Site_Name))){
    ...
    df <- img_list %>% 
      ...
      dplyr::filter(out_class == 4 & 
                      SPC20Unknown >30 )   %>%        # Filter the data for a specific class
      as_tibble()
...
plot <- output %>% 
  ...
  reframe(extent = sum(SPC20Unknown)*(10^-6),
          n = n()) %>% 
  ggplot()+
  geom_point(aes(x = year, y = (n*100)*(10^-6), color = site, group = site))+
  ...
  geom_label(aes(x = 2016, y = 1 , label = "A"), size = 10)+
  # scale_y_log10() +
...
plot_SPC <- output %>% 
  ggplot() + 
  geom_boxplot(aes(x = year, y = SPC20Unknown, color = site, group = interaction(site,year)), alpha = 0.1) +
  # geom_smooth(aes(x = year, y = SPC20Unknown, color = site, fill = site, group = site), 
  #             method = "gam",
  #             formula = y ~ s(x, k = 8),
  #             alpha = 0.1
  # )+
  geom_label(aes(x = 2016, y = 90 , label = "B"), size = 10)+

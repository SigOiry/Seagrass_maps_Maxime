...
\text{where:} \\\quad \text{Extent}_{s,d} \text{ is the total seagrass extent at site } s \text{ and date } d, \\\quad N_{s,d} \text{ is the number of pixels classified as seagrass at site } s \text{ and date } d, \\\quad A_{\text{pixel}} = 100 \text{ m}^2 \text{ is the area of one pixel}, \\\quad D_i \text{ is the seagrass density of pixel } i \text{ (scaled from 0 to 1)}.
$$
...
plots <- map(years, function(yr) {
  base_plot <- ggplot() +
    geom_raster(data = df_rgb %>% filter(site == "Fort Espagnol", year == yr),
                aes(x = x, y = y, fill = rgb(R, G, B, maxColorValue = 0.9))) +
    scale_fill_identity() +
    new_scale_fill() +
...
plots <- map(years, function(yr) {
  base_plot <- ggplot() +
    geom_tile(data = df_rgb %>% filter(site == "Kerouarc'h", year == yr),
                aes(x = x, y = y, fill = rgb(R, G, B, maxColorValue = 0.9))) +
    scale_fill_identity() +
    new_scale_fill() +
...
ggsave("Manuscript/Figs/plot_all.pdf", plot = plot_all, device = cairo_pdf, width = 10, height = 7, dpi = 300)
...
#| echo: false
#| warning: false
#| fig-cap: "Time Series of Seagrass Percent Cover from 2016 to 2023 in Fort Espagnol and Kerouarc'h, two sites along the Auray River. Red points represent sampling stations: 4B - 47.616°N, 2.953°W; 2C - 47.583°N, 2.955°W. SPC values at the sampling points are provided at the bottom left corner of the plot if SPC is greater than 20%."
#| label: fig-Maps
#| fig-width: 10
#| fig-height: 8
#| out-width: "100%"
...
ggplot(filelist, aes(x = date, y = 0)) +
  geom_point(size = 3) +
  # geom_label(aes(label = date), vjust = -1.2, size = 3) +
  geom_mark_ellipse(aes(group = date, label = date), fill = NA, label.fill = NA) +
  ...
  labs(x = "Date", y = NULL, title = "1D Time Diagram with Grouped Ellipses")
```

```{r extent retrieving overtime}
#| cache: false
#| echo: false
#| warning: false
#| eval: false
...
ggplot( aes(x = date, y = area_totale)) +
  geom_point()+
  geom_line()
...
df_sp_SPC %>% 
  mutate(doy = yday(date)) %>% 
ggplot( aes(x = date, y = relative_area)) +
  geom_point()+
  geom_line()
```

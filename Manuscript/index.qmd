...
The uncertainty for each image was calculated as the inverse of the mean of the per pixel probabilities from the ICES CREAMS model ($\bar{p}$) divided by the global accuracy when applied to validation data ($G_{\alpha}$). 
...
plot <- output %>% 
  ...
  reframe(seagrass_area = n() * 100 * (10^-6),
          prob = (1-mean(class_probs, na.rm = T))/(0.91)) %>%
  ungroup() %>% 
  group_by(site) %>% 
  mutate(relative_surface = seagrass_area / max(seagrass_area), 
         relative_prob = prob*relative_surface) %>% 
  # left_join(area, by = "site") %>%
  ggplot() +
  geom_errorbar(aes(x = year,
                    y = relative_surface,
                    ymin = relative_surface-relative_prob,
                    ymax = relative_prob+relative_surface,
                    color = site ),
...
for (i in 1:nrow(filelist)) {
  ...
  if(i == 1){
    stk_prob <- rast(filelist$path[i]) %>% 
      dplyr::filter(SPC > 20)
    stk_prob <- stk_prob$class_probs
  }else{
    a <- rast(filelist$path[i]) %>% 
      dplyr::filter(SPC > 20)
    a <- a$class_probs
    
    stk_prob <- c(stk_prob,a)
  }
  
}

names(stk_prob) <- filelist$date
...
stk_sp_prob <- stk_prob %>% 
  crop(sp_mask, mask = T)

stk_ker_prob <- stk_prob %>% 
  crop(ker_mask, mask = T)
...
df_ker_prob <- stk_ker_prob %>% 
  as.data.frame(xy = T) %>% 
  pivot_longer(-c(x,y),names_to = "date", values_to = "prob") %>% 
  group_by(date) %>% 
  reframe(uncertainty = (1-mean(prob, na.rm = T))/0.91) %>% 
  ungroup() %>%
  mutate(date = as.Date(date,format = "%Y-%m-%d"),
         site = "Kerouarc'h",
         uncertainty = case_when(is.nan(uncertainty) ~ 0,
                                 T ~ uncertainty))
...
df_sp <- df_sp_SPC %>% 
  left_join(df_sp_prob, by = c("date","site"))

df_ker <- df_ker_SPC %>% 
  left_join(df_ker_prob, by = c("date","site"))

df_all <- rbind(df_sp,df_ker) %>% 
  mutate(relative_area = relative_area*0.0001, 
         ymin = relative_area - (relative_area*uncertainty),
         ymax = relative_area + (relative_area*uncertainty))

(Fig_3 <- df_all %>% 
  mutate(site = case_when(site == "Kerouarc'h" ~ "Kerouarc'h (2C)",
                          T ~ "Fort Espagnol (4B)")) %>%  
  mutate(doy = yday(date)) %>% 
  ggplot(aes(x = date, y = relative_area, ymin = ymin, ymax = ymax, color = site, fill = site)) +
  geom_point(show.legend = F, shape = 21, size = 3, color = "black") +
  geom_line(show.legend = F) +
  geom_errorbar(show.legend = F)+
  scale_color_manual(values = cols, name = "Study Site: ") +
  scale_fill_manual(values = cols, name = "Study Site: ") +
  scale_y_continuous(
    name = "Area (ha)") +
  theme_Bede()+
...
#| echo: false
#| warning: false
#| fig-cap: "Evolution of the seagrass cover at Fort Espagnol (left) and at Kerouarc'h (right) during 2022 - 2023. Points with error bars show neural network estimated cumulative area and average uncertainty per satellite image."
#| label: fig-Pheno
#| fig-width: 10
#| fig-height: 5.5
#| out-width: "100%"
